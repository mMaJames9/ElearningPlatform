                            ...
                            <div class="col-12 col-lg d-flex align-items-center my-3">
                                ...
                                <p class="flex-1">
                                    <form method="POST" action="{{ route('plans.destroy', $currentSubscription->plan) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-falcon-info me-1 mb-1">Cancel plan</button>
                                    </form>
                                </p>
                                @endif
                            </div>
                        ...
                        <div class="col-md">
                            <div class="border rounded-3 overflow-hidden">
                                ...
                                <div class="p-4 bg-light">
                                    ...
                                    <form method="POST" action="{{ route('plans.update', 1) }}">
                                        @csrf
                                        @method('PUT')
                                        <button class="btn btn-primary d-block w-100 mt-5">{{__('Purchase Now')}}</button>
                                    </form>
                                </div>
                            </div>
                        </div>
